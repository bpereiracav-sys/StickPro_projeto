...
function getClubSide(match, teamName) {
  ...
  if (match.home_team === teamName) return 'home';
  if (match.away_team === teamName) return 'away';
  return null;
}
...
function calculateStatistics(matches = [], teamName = '') {
  ...
  clubMatches.forEach((match) => {
    ...
    const clubScore = side === 'away' ? awayScore : homeScore;
  ...
  return {
    ...
    goalsPerGame: played > 0 ? (goalsFor / played).toFixed(1) : '0.0',
      ...
      <Card className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-sm transition-all duration-300 hover:shadow-md">
        ...
        <CardContent className="p-5 sm:p-6">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard label="Taxa de vitória" value={`${stats.winRate}%`} icon={TrendingUp} />
            <MetricCard label="Golos por jogo" value={stats.goalsPerGame} icon={Goal} />
            <MetricCard label="Golos marcados" value={stats.goalsFor} icon={Target} />
            <MetricCard label="Golos sofridos" value={stats.goalsAgainst} icon={Shield} />
            <MetricCard label="Diferença de golos" value={stats.goalDifference > 0 ? `+${stats.goalDifference}` : stats.goalDifference} icon={Sparkles} />
            <MetricCard label="Maior vitória" value={stats.biggestWin} icon={Trophy} />
          </div>

          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-2xl bg-emerald-50 p-4 text-center">
              <p className="text-xs uppercase tracking-wide text-emerald-700">Vitórias</p>
              <p className="mt-1 text-xl font-bold text-emerald-800">{stats.wins}</p>
            </div>

            <div className="rounded-2xl bg-amber-50 p-4 text-center">
              <p className="text-xs uppercase tracking-wide text-amber-700">Empates</p>
              <p className="mt-1 text-xl font-bold text-amber-800">{stats.draws}</p>
            </div>

            <div className="rounded-2xl bg-red-50 p-4 text-center">
              <p className="text-xs uppercase tracking-wide text-red-700">Derrotas</p>
              <p className="mt-1 text-xl font-bold text-red-800">{stats.losses}</p>
            </div>

            <div className="rounded-2xl bg-cyan-50 p-4 text-center">
              <p className="text-xs uppercase tracking-wide text-cyan-700">Clean sheets</p>
              <p className="mt-1 text-xl font-bold text-cyan-800">{stats.cleanSheets}</p>
            </div>
          </div>
